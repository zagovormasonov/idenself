  ...
  try {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        symptoms: [], // Will be updated later
      })
      .select()
      .single();
    ...
    res.json({ sessionId: data.id, symptoms });
  } catch (error) {
...
app.post('/api/survey/:id/submit-symptoms', async (req, res) => {
  try {
    ...
    const { error: updateError } = await supabase
      .from('sessions')
      .update({
        status: 'PART1_STARTED',
        symptoms,
        complaint: generalDescription
      })
      .eq('id', id);
...
app.post('/api/survey/:id/submit', async (req, res) => {
  try {
    ...
    if (session.status === 'PART1_STARTED') {
      ...
      await supabase
        .from('sessions')
        .update({ status: 'PART2_STARTED' })
        .eq('id', id);

      res.json({ nextStep: 'PART2', questions: questionsPart2 });

    } else if (session.status === 'PART2_STARTED') {
      // Similar logic for Part 2 -> Part 3 or Results
      // ... (implement similar to NestJS logic)

      res.json({ nextStep: 'FINISHED' });
    }
...
async function generatePart1(symptoms, generalDescription) {
  // Use existing generate-part1 endpoint logic
